...
rec_A_pose = {'x': 3.5, 'y': 3.5, 'yaw': -pi/2}
rec_B_pose = {'x': 3.5, 'y': -3.5, 'yaw': pi}
rec_C_pose = {'x': -3.5, 'y': -3.5, 'yaw': pi/2}
rec_D_pose = {'x': -3.5, 'y': 3.5, 'yaw': 0}
phi = atan(1/7)
...
def main():
    ...
    rospy.Subscriber('/my_robot/cmd_vel', Twist, subcriber_vel_callback)
    cnt = 0
    step = 0
    rate = rospy.Rate(50)
    while not rospy.is_shutdown():
